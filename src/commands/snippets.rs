    ...
    for entry in fs::read_dir(snippets_dir).context("Failed to read snippets directory")? {
        ...
        if path.extension().is_some_and(|ext| ext == "yaml") {
            ...
            crate::commands::ensure_filename_matches_identity(
                &path,
                &format!("{}-{}", metadata.id, metadata.trigger),
                "trigger",
            )?;
...
mod tests {
    ...
    #[test]
    fn test_get_all_snippet_metadata_from_path_rejects_filename_not_matching_trigger() {
        let temp_dir = TempDir::new().unwrap();
        let dir = temp_dir.path();
        ...
        assert!(err_msg.contains("rename this file and its .sql to 0-same_trigger.*"));
    }
